...
const TaskList = () => {
    ...
    const [tasks, setTasks] = useState<Task[]>(() => {
        try {
            ...
            const parsed = JSON.parse(saved) as SerializedTask[];
    ...
    const handleToggleComplete = (id: string) => {
        setTasks((prev) =>
            prev.map((task) =>
                task.id === id ? { ...task, completed: !task.completed, urgent: false } : task
            )
        );
    };
    ...
    const handleEditSave = (id: string) => {
        setTasks((prev) =>
            prev.map((task) => (task.id === id ? { ...task, description: editingDescription } : task))
        );
        setEditingId(null);
    ...
    // Would be nice to change the buttons to icons if I have the time and add proper WCAG labels
    return (
        <main className="px-4 py-10 text-center flex flex-col min-h-screen">
            ...
            <form onSubmit={handleSubmit} className="mb-6">
                <label htmlFor="new-task" className="text-lg font-medium">
                    New Task:
                </label>
                <input
                    id="new-task"
            ...
            <ul className="mb-6">
                {tasks.map((task) => (
                    ...
                    >
                        <span className="w-40 text-left text-sm text-gray-500">{task.createdAt.toLocaleString()}</span>
                        ...
                        <span className="flex-1 text-center px-4">
                            {editingId === task.id ? (
                                ...
                                <>
                                    ...
                                    {task.urgent ? <span className="ml-2 text-red-700 font-bold">Urgent</span> : null}
                                </>
                            )}
                        </span>
                        ...
                        <span className="flex items-center gap-2">
                            {!task.completed && editingId !== task.id && (
                                <button onClick={() => handleEdit(task)} className="p-1 border rounded bg-yellow-500 text-white">
                                    Edit
                                </button>
                            )}
                            <button onClick={() => handleDelete(task.id)} className="p-1 border rounded bg-red-500 text-white">
                                Delete
                            </button>
                            <input type="checkbox" checked={task.completed} onChange={() => handleToggleComplete(task.id)} className="ml-2" />
                        </span>
                    </li>
                ))}
            </ul>
